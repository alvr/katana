...
suspend fun <D : Operation.Data> ApolloCall<D>.executeOrThrow() =
    execute().also { response -> response.exception?.let { throw it } }

fun <D : Query.Data> ApolloCall<D>.watchFiltered() = watch().filterNot { it.exception is CacheMissException }

val <V> V?.optional
    get(): Optional<V?> = Optional.presentIfNotNull(this)

val <V> V.present
    get(): Optional<V?> = Optional.present(this)

val <V> Either<*, V>.optional
    get(): Optional<V?> = Optional.presentIfNotNull(getOrNull())

val <V> Option<V>.optional
    get(): Optional<V?> = Optional.presentIfNotNull(getOrNull())
