...
internal class ListsViewModelTest : FreeSpec() {
    private val observeList = mockk<ObserveListUseCase>()
                            ...
                            "initializing viewModel" -
                                {
                                    "the collections observed are empty" {
                                        ...
                                        coJustRun { observeList(scenario.type) }
                                        ...
                                        coVerify(exactly = 1) { observeList(scenario.type) }
                                        verify(exactly = 1) { observeList.flow }
                                    }
                            ...
                            "refreshing" -
                                {
                                    "is successful" {
                                        ...
                                        coVerify(exactly = 2) { observeList(scenario.type) }
                                        verify(exactly = 2) { observeList.flow }
                                    }

                                    "is failure" {
                                        coJustRun { observeList(scenario.type) }
                                        every { observeList.flow } returnsMany
                                            listOf(
