                ...
                "the collections are empty" -
                    {
                        "the collections observed should are empty" {
                            every { observeAnime.flow } returns
                                flowOf(MediaCollection<MediaEntry.Anime>(lists = emptyList()).right())
        ...
        "searching" -
            {
                ...
                "try to select a non-existent anime list" {
                    ...
                    viewModel.test {
                        ...
                        expectState {
                            ...
                            copy(loading = false, selectedList = "NonExistent Anime List")
                        }
                    }
                ...
                listOf(Triple("non-existent entry", true, persistentListOf<MediaListItem.AnimeListItem>())).forEach {
                    (text, empty, result) ->
        ...
        "refreshing" -
            {
                ...
                "is failure" {
                    coJustRun { observeAnime() }
                    every { observeAnime.flow } returnsMany
                        listOf(
                            flowOf(
                                MediaCollection(
                                        lists =
        ...
        every { observeAnime.flow } returns
            flowOf(
                MediaCollection(
                        lists =
                            listOf(
                                MediaListGroup(name = "MyCustomAnimeList", entries = listOf(animeMediaEntry1)),
                                MediaListGroup(name = "MyCustomAnimeList2", entries = listOf(animeMediaEntry2)),
                            )
                    )
                    .right()
            )
