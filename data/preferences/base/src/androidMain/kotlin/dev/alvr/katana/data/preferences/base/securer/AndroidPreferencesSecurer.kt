package dev.alvr.katana.data.preferences.base.securer

import androidx.datastore.core.CorruptionException
import com.google.crypto.tink.Aead
import java.security.GeneralSecurityException

internal class AndroidPreferencesSecurer(
    private val aead: Aead,
) : PreferencesSecurer {
    override fun fromSecured(input: ByteArray): ByteArray = operation("Aead decrypt") {
        aead.decrypt(input, null)
    }

    override fun toSecured(input: ByteArray): ByteArray = operation("Aead encrypt") {
        aead.encrypt(input, null)
    }

    private inline fun <R> operation(message: String, block: () -> R): R = try {
        block()
    } catch (e: GeneralSecurityException) {
        throw CorruptionException(message, e)
    }
}
